...
export async function handler(event) {
  try {
    ...
    const system = {
      ...
      status: clean(payload.status) === "online" ? "online" : "offline",
      actionText: clean(payload.actionText) || (clean(payload.status) === "online" ? "进入查询" : "即将开放"),
    ...
    if (!system.id || !system.title) {
      return {
        statusCode: 400,
        headers: { "Content-Type": "application/json; charset=utf-8" },
        body: JSON.stringify({ error: "入口 ID 和入口名称不能为空。" })
      };
    }
    ...
    if (index >= 0) systems[index] = system;
    ...
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json; charset=utf-8" },
      body: JSON.stringify({ error: "保存入口配置失败。" })
    };
  }
}
